...
class CBitrixPlayer extends CBitrixComponent
{
	...
	public function GetContentType ($src)
	{
		...
		if ($type == "application/octet-stream" || $type == "")
		{
			...
			$type = $arTypes[strtolower(substr($src, bxstrrpos($src, ".") + 1))];
			if ($type == "")
			{
	...
	public function executeComponent()
	{
		...
		$ext = strtolower(GetFileExtension($this->path));
		...
		else
		{
			...
			$this->arResult['FLASH'] = $this->arResult['STREAM'] = $this->arResult['COMMON_VIDEO'] = $this->arResult['YOUTUBE'] = $this->arResult['AUDIO_FILE'] = false;
	...
	protected function processJwPlayer()
	{
		...
		// Skining
		$skinExt = strtolower(GetFileExtension($this->arParams['SKIN']));
	...
	protected function processTrack (&$arTrack)
	{
		if ($this->arResult['VIMEO'])
		{
			$this->warning .= GetMessage("PLAYLIST_VIMEO_NOT_SUPPORTED")."<br />";
			return false;
		}
		if ($this->arResult['STREAM'])
		{
		...
		{
			$this->warning .= $arTrack['src'].": ".GetMessage("PLAYLIST_FILE_NOT_FOUND")."<br />";
			return false;
		}
	...
	protected function processPlaylist()
	{
		...
		{
			// original plugin doesn't support multiple players
