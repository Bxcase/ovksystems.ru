		...
		<div class='tovar' id='".$this->GetEditAreaId($arItem['ID'])."'>
			...
			<div class='preview'>
				<div class='svoistva'>
					...
					$cena=str_replace(array(" ", ","), array("", "."), $arItem["PROPERTIES"]["CENA"]["VALUE"]); // пытаемся преобразовать цену в число
					if (is_numeric($cena)==1) { // если эта строка похожа на число, то приводим его к нужному формату
						$cena=stroka_v_chsilo($arItem["PROPERTIES"]["CENA"]["VALUE"])." руб.";
					} else { // иначе берем как есть
						$cena=$arItem["PROPERTIES"]["CENA"]["VALUE"];
					}
